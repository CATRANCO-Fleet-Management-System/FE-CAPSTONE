...
const RecordBox = ({ busId, maintenanceStatus, description, onDelete }) => {
  ...
  const toggleDescription = () => setIsDescriptionVisible(!isDescriptionVisible); // Toggle description
...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  const createPageButtons = () => {
    const pageButtons = visiblePages.filter((p) => p <= totalPages).map((p) => (
      <button
        key={p}
...
const DashboardHeader = () => {
  ...
  const busRecordsData = [
    { id: "Bus 1", status: "On Maintenance", description: "Overhaul all parts and systems" },
    { id: "Bus 2", status: "Off Maintenance", description: "Routine inspection completed" },
    { id: "Bus 3", status: "On Maintenance", description: "Engine and brake system repair" },
    { id: "Bus 4", status: "Off Maintenance", description: "Electrical system upgrade" },
  ];
  ...
  const handleAddNewClick = () => {
    router.push("bus-maintenance/addpage"); // Navigate to the add-page.tsx
  };
        ...
        <div className="content flex flex-col flex-1">
          ...
          <div className="records flex flex-col h-full">
            <div className="output flex mt-4 items-center ml-14">
              <Records busRecords={filteredBusRecords} onDelete={handleDelete} />
            </div>
